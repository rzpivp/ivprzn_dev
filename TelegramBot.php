...
//public function command_commandname($params, $message) {}
  ...
  protected function apiSendObj($method, $obj, $key, $text='') {
    $params = ['chat_id' => $this->chatId, $key	=> $obj];
...
//print_r($ret);
    $ret = false;
    if ($res['ok']) {
      $src = $this->core->apiUrl . '/' . $res['result']['file_path'];
...
echo '---'.$src.'---'.$dest.'----'      ;
      if ($ret) {
        $this->apiSendMessage('Фото сохранено');
        $sql = 'SET NAMES utf8;';
        $caption = htmlspecialchars($message['message']['caption']);
        $sql .= "INSERT IGNORE INTO {$cnfg['tlgrmlogsdata']} (`user_id`,`type_mess`,`value`, `caption`) VALUES('{$user_id}','{$method}','{$dest}','{$caption}');";
